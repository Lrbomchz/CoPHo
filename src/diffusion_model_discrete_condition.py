...
class DiscreteDenoisingDiffusion(pl.LightningModule):
    ...
    def on_test_epoch_end(self) -> None:
        ...
        samples_left_to_generate = 40
        samples_left_to_save = 40
        ...
        for i in range(1, 10):
            filename = (f"generated_samples|delta{self.condi_delta}|weight{self.condition_weight}|"
                        f"multiplier{self.sampling_multiplier}|phase{self.condition_phase_s}-{self.condition_phase_e}|"
        ...
        # 设置一个新的文件名，用于保存中间链信息
        for i in range(1, 10):
            chain_filename = (f"intermediate_chains|delta{self.condi_delta}|weight{self.condition_weight}|"
                              f"multiplier{self.sampling_multiplier}|phase{self.condition_phase_s}-{self.condition_phase_e}|"
                              f"mode{self.condition_mode}-{i}.txt")
    ...
    def sample_batch(self, batch_id: int, batch_size: int, keep_chain: int, number_chain_steps: int,
        ...
        for s_int in reversed(range(0, self.T)):
            ...
            # Sample z_s
            sampled_s, discrete_sampled_s = self.sample_p_zs_given_zt(s_norm, t_norm, X, E, y, node_mask, cond)
    ...
    def cond_fn__(self, noisy_data, node_mask, target=None):
        with torch.enable_grad():
            ...
            for i in range(bs):
                ...
                prob = target_flat / (target_flat.sum() + 1e-8)
                sampled_indices = torch.multinomial(prob, self.condi_delta, replacement=False)
                #sampled_indices =
            ...
            delta = self.condi_delta
            src_idx_flat = src_idx.view(-1)
            tgt_idx_flat = tgt_idx.view(-1)
            dist_flat = dist.view(-1)

            # 将 E_v_ind 复制、分离并设置 requires_grad=True，确保其独立于原图
            E_v_rep = E_v_ind.repeat_interleave(delta, dim=0).clone().detach().float().requires_grad_(True)

            # 前向传播，调用 guidance_model，注意这里会触发各个 hook 打印中间输出信息
            pred = self.guidance_model(E_v_rep, src_idx_flat, tgt_idx_flat)

            # 计算损失并反向传播
            loss = F.mse_loss(pred, dist_flat)
            loss.backward()

            # 计算并返回最终梯度（示例中保持原有逻辑）
            grad_rep = E_v_rep.grad.detach()
            grad_rep = grad_rep.view(bs, delta, n, n)
    ...
    def cond_fn(self, noisy_data, node_mask, target=None):
        with torch.enable_grad():
            ...
            for i in range(bs):
                ...
                topk = torch.topk(stable_target_upper, 1500, largest=True)
                # 固定取第800和第1000个采样
                sampled_relative_indices = torch.stack([topk.indices[999]])
                # 将相对索引映射为原始展平索引
    ...
    def sample_p_zs_given_zt(self, s, t, X_t, E_t, y_t, node_mask, cond=None):
        ...
        # print("[DEBUG]")
        # print(X_t.shape, E_t.shape, y_t.shape, node_mask.shape)
        extra_data = self.compute_extra_data(noisy_data)
        pred = self.forward(noisy_data, extra_data, node_mask)
